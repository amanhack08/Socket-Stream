...
public class WiFiDirectBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent)
    {
        ...
        else if(WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(action))
        {
            if (manager != null)
            {
                Toast.makeText(activity,"Receiver",Toast.LENGTH_SHORT).show();
                if (ActivityCompat.checkSelfPermission(activity, ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED)
                {
                    manager.requestPeers(channel, (WifiP2pManager.PeerListListener) activity.getSupportFragmentManager().findFragmentById(R.id.list_devices));
                }
                else
                {
        ...
        else if (WifiP2pManager.WIFI_P2P_CONNECTION_CHANGED_ACTION.equals(action))
        {
            ...
            if (networkInfo.isConnected()) {
                DeviceDetailsFragment fragment = (DeviceDetailsFragment) activity.getSupportFragmentManager().findFragmentById(R.id.details_device);
        ...
        {
            DevicesListFragment fragment = (DevicesListFragment) activity.getSupportFragmentManager().findFragmentById(R.id.list_devices);
