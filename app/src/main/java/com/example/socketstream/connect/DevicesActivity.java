...
import com.example.socketstream.connect.DevicesListFragment;
...
{
    public static final String TAG = "socketstream";
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
       ...
       // fileExtension = getIntent().getExtras().getString("extension");
        ...
        if(check.equals("3")) {
            ArrayList<Uri> uriArrayList = (ArrayList<Uri>) getIntent().getSerializableExtra("mylist");
            stringUriList = new ArrayList<>();
            for (int i = 0; i < uriArrayList.size(); i++)
                stringUriList.add(uriArrayList.get(i).toString());
        }else{
            stringUriList=new ArrayList<>();
        }
        fragmentList = (DevicesListFragment) getSupportFragmentManager().findFragmentById(R.id.list_devices);
        fragmentDetails = (DeviceDetailsFragment) getSupportFragmentManager().findFragmentById(R.id.details_device);
                        ...
                        public void onFailure(int reasonCode) {
                            switch (reasonCode) {
                                ...
                                case WifiP2pManager.ERROR:
                                    ...
                                    Toast.makeText(DevicesActivity.this, "Getting error while peers discover", Toast.LENGTH_SHORT).show();
                                    break;

                                case WifiP2pManager.P2P_UNSUPPORTED:
                                    Toast.makeText(DevicesActivity.this, "Device is not supported", Toast.LENGTH_SHORT).show();
                                    break;


                                case WifiP2pManager.BUSY:
                                    Toast.makeText(DevicesActivity.this, "Device is busy", Toast.LENGTH_SHORT).show();
                                    break;
